-- Replace backfill functions with batched versions to avoid statement timeout
DROP FUNCTION IF EXISTS backfill_mecklenburg_sale_dates();
DROP FUNCTION IF EXISTS backfill_mecklenburg_deed_dates();
DROP FUNCTION IF EXISTS backfill_mecklenburg_zips();

-- Batched sale date backfill (processes 5000 rows at a time)
